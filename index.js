class Color{
    ...
    get lchabString(){
        ...
        return (this.capitalize) ? "LCHab("+truncLCHAB.join(", ")+")" : "lchAB("+truncLCHAB.join(", ")+")";
    }
    set lchab(lchab){
    ...
    static rgbToXyz(rgb){
        ...
        let cG = rgb[1]/255;
        let cB = rgb[2]/255;
        let invCompand = (c) => c<= 0.04045 ? c/12.92 : Math.pow((c+0.055)/1.055,2.4);
    ...
    static xyzToRgb(xyz){
        ...
        let compand = (c) => c<=0.0031308 ? 12.92*c : 1.055*Math.pow(c,1/2.4)-0.055;
        let cR = compand(invR);
        let cG = compand(invG);
        let cB = compand(invB);
    ...
    static labToLCHab(lab){
        ...
        let h = Math.atan2(b,a)>=0 ? Math.atan2(b,a)/Math.PI*180 : Math.atan2(b,a)/Math.PI*180 + 360;
        return [lab[0],c,h]; 
    }
